...
### On-device UMAAL ASM differential testing

The UMAAL Montgomery multiply is hand-written ARMv8-M assembly. Unit tests on the host only prove it works on x86. `bench-rp2350-m33-bn-asm-test` is a dedicated firmware that calls `bn::arith::selftest_fq` and `selftest_fr` (behind the `test-harness` feature in vendor/bn) directly on the Pico 2 W.

Both functions use an inline splitmix64 PRNG to generate random inputs, run both the UMAAL asm path and the pure-Rust reference path, and compare outputs byte-for-byte. Firmware prints `PASS` over USB serial. If there is any divergence it prints the failing input and halts with `FAIL`.

This closes the "passes on x86, blows up on M33" class of bugs. The PRNG seed is fixed so the test is deterministic and reproducible.

### STARK prover self-verification

The on-device STARK prover firmwares (Phase 4 and Phase 5) immediately verify the proof they generated on the same device before reporting success. If `winterfell::verify` returns anything other than `Ok`, the firmware halts. Every reported "PROVE ok" is backed by a passing "VERIFY ok" on actual silicon. Proof correctness is not assumed from the host prover.
