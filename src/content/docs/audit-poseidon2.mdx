...
Plus `tests/perm_diff.rs` which runs both Plonky3's perm and ours on the same constants and asserts byte-identical output.
...
Paper page 12 says M_4 is MDS for `p > 2^31`. `BabyBear`'s prime is `15·2^27 + 1 = 2_013_265_921` which is LESS than `2^31`. So the paper's claim does not directly cover us.
