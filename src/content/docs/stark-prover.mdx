...
<div class="hero-badges">
  <Badge text="winterfell 0.13 (forked)" variant="note" />
  <Badge text="on-device prove + self-verify" variant="note" />
  <Badge text={`${fmtMs(usAt('stark_m33_prove_bb'))} ms at 95-bit security`} variant="success" />
  <Badge text="N=256 SRAM ceiling confirmed" variant="caution" />
  <Badge text="Blake3 is the ISA gap floor" variant="tip" />
</div>
...
Compare that against Fibonacci N=256 (same field, same security target, same 11 queries):

| | Fibonacci N=256 | Threshold N=64 | **delta** |
|-|---:|---:|---:|
| prove | {fmtMs(usAt('stark_m33_prove_bb'))} ms | {fmtMs(usAt('threshold_m33_prove'))} ms | **3.1× faster** |
| verify | {fmtMs(usAt('stark_m33_verify_bb'))} ms | {fmtMs(usAt('threshold_m33_verify'))} ms | 1.1× faster |
| heap | 248 KB | 64 KB | **3.9× less** |

The verify speedup is only 1.1× even though the trace is 4× shorter. Verifier cost at 11 queries is FRI-dominated — opening 11 Merkle paths costs the same regardless of trace length. Prove and heap scale roughly with the trace, verifier barely moves.

So yeah the prover scales nicely, the verifier is query-bound. If you want a faster verifier you need fewer queries or a larger blowup factor, not a shorter trace.
