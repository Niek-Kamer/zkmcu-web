...
<div class="hero-badges">
  <Badge text="Self-audit: I wrote it, I reviewed it" variant="caution" />
  <Badge text="Zero blockers in main" variant="success" />
  <Badge text="Failures published" variant="tip" />
</div>
...
- **Main is clean**: Found zero blockers or "should-fix" issues in the latest `main` across all three verifiers, the UMAAL asm, and the Winterfell fork.
...
**Hand-rolled ASM differential testing.** Writing ARMv8-M assembly for Montgomery multiplication is "cowboy territory" unless you can prove it's right. I cross-check the ASM output against a pure Rust reference implementation (`mul_reduce_u32_ref`) using randomized inputs. It matches, byte-for-byte.
...
- **100+ Adversarial Tests**: I ran 84 tests on Groth16 and 19 on the STARK wrapper. This includes flipping every single bit in a valid proof to make sure _nothing_ produces a false positive `Ok(true)`.
